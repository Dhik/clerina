...
class NetProfitController extends Controller
{
    ...
    public function exportUnknownOrders()
    {
        $newSpreadsheetId = '1Ukssd8FRbGA6Pa_Rsn3FJ2SP_W2CS4rkIhh3o5yw1gQ';
        ...
        // Query to get all orders with customer_name = "unknown"
        ...
        // Add headers based on orders table structure
        $data[] = [
            'ID',
            'Date',
            'Process At',
        ...
        foreach ($orders as $order) {
            $data[] = [
                ...
                $order->process_at ? Carbon::parse($order->process_at)->format('Y-m-d') : '',
                $order->id_order,
                ...
                $order->updated_at ? Carbon::parse($order->updated_at)->format('Y-m-d H:i:s') : '',
                ...
                $order->is_booking,
                $order->status,
                $order->success_date ? Carbon::parse($order->success_date)->format('Y-m-d') : ''
            ];
        }
        ...
        return response()->json([
            'success' => true, 
            'message' => 'Unknown customer orders exported successfully to Google Sheets',
            'count' => count($data) - 1 // Subtract 1 for header row
        ]);
    }
